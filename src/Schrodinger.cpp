 ...
 * @param z mesh_hermite on which to evaluate the solution
 * @return the solution computed along given mesh_hermite
 */
arma::colvec Schrodinger::psi(int n, arma::colvec z) {
    Poly poly{};
    poly.calcHermite(n, sqrt(m * w / h_bar) * z);
    return (1 / sqrt(pow(2, n) * MathTools::factorial(n))) * pow((m*w)/(pi*h_bar), 0.25) * arma::exp(-m*w*(z%z) / (2*h_bar)) % poly.hermite(n);
}
 ...
 *
 * @param n energy level
 * @param z mesh_hermite on which to evaluate the derivative
 * @return the second derivative of the solution, evaluated along given mesh_hermite
 */
 ...
 * @param n energy level
 * @param z mesh_hermite on which to evaluate the energy
 * @return the estimation of the energy evaluated along given mesh_hermite
 */
 ...
 *
 * Computes integral scalar product between two energy-level solutions using Gauss-Poly quadrature method.
